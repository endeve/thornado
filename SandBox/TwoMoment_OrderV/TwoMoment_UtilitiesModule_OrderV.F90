...
!! Mathi: Rich 1
    ...
    DO WHILE( ANY( ITERATE ) .AND. k < MaxIterations )
      ...
      k = k + 1
      Mk = MIN( M, k )  !! Mathi: So possible max of Mk is M that is 2
      CALL TimersStart( Timer_Streaming_NumericalFlux_RHS )

!! Mathi: Rich 2
#if   defined( THORNADO_OMP_OL )
      !!$OMP NUM_TEAMS((4*nZ+team_size-1)/team_size) THREAD_LIMIT(team_size) &  
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SIMD &
...
!! Mathi: Possible benefit of unrolling
!#if   defined( THORNADO_OMP_OL )
...
!! tested SIMDLEN(1/8/16) all produce NaN in the Two-Moment Tally at Cycle = 00000001
        ...
        !$OMP NUM_TEAMS((4*nZ*Mk+team_size-1)/team_size) THREAD_LIMIT(team_size) &  
        !$OMP PRIVATE( SUM1 )
#elif defined( THORNADO_OACC   )
        !$ACC PARALLEL LOOP GANG VECTOR COLLAPSE(2) &
        !$ACC PRIVATE( SUM1 ) &
...
!! Mathi: Rich 4
      ...
      team_size = 128
      !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO SIMD &
      !$OMP NUM_TEAMS((4*nZ*(Mk-1)+team_size-1)/team_size) THREAD_LIMIT(team_size) &  
      !$OMP PRIVATE( iX, CONVERGED, FTMP, GTMP )
#elif defined( THORNADO_OACC   )
      !$ACC PARALLEL LOOP GANG VECTOR &
      !$ACC PRIVATE( iX, CONVERGED, FTMP, GTMP ) &
