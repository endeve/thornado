...
CONTAINS
    ...
    SELECT CASE( iDim )
      ...
      CASE( 1 )
        ...
        IF( DEBUG )THEN
          ...
          CALL ComputeFluxJacConsMatrix( U, G, dFdU )
  ...
  SUBROUTINE ComputeFluxJacConsMatrix( U, G, dFdU )
    ...
    REAL(DP) :: dUdV(nCF,nCF), FluxJacPrim(nCF,nCF)
    ...
    FluxJacPrim(:,1) = [ Vu1 * W - eta * W, &
                         ...
                         ( One + epsilon + chi ) * W**2 * Vu1 - W * Vu1 &
                           ...
                           - One ) ), &
                         Zero ]
    FluxJacPrim(:,2) = [ rho * W * ( W**2 * Vu1 * Vd1 + One ) &
                           - eta * rho * W**3 * Vd1, &
    ...
    FluxJacPrim(:,3) = [ rho * W * ( W**2 * Vu1 * Vd2       ) &
                           - eta * rho * W**3 * Vd2, &
                         ...
                         Zero ]
    FluxJacPrim(:,4) = [ rho * W * ( W**2 * Vu1 * Vd3       ) &
                           - eta * rho * W**3 * Vd3, &
    ...
    dFdU = MATMUL( FluxJacPrim, inv(dUdV) )
