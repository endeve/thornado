MODULE MF_TimersModule_MHD
  ...
  USE MF_MHD_TimersModule, ONLY: &
    TimeIt_AMReX_MHD, &
    InitializeTimers_AMReX_MHD, &
    FinalizeTimers_AMReX_MHD
  ...
  REAL(DP), PUBLIC :: Timer_AMReX_ComputeTimeStep_MHD; INTEGER :: iT_CTE = 3
...
CONTAINS
  ...
  SUBROUTINE InitializeTimers_AMReX( WriteMMS_Option )
    ...
    Timer_AMReX_ComputeTimeStep_MHD = Zero
    ...
    IF( TimeIt_AMReX_MHD ) &
      CALL InitializeTimers_AMReX_MHD &
  ...
  SUBROUTINE FinalizeTimers_AMReX( RestartProgramTimer_Option, Verbose_Option )
    ...
    Timer(iT_CTE) = Timer_AMReX_ComputeTimeStep_MHD
    ...
    IF( Verbose )THEN
      ...
      Label(iT_CTE) = 'Compute Timestep (MHD)'
...
END MODULE MF_TimersModule_MHD
