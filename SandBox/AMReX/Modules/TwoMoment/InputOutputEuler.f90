MODULE InputOutputEuler
  ...
  PUBLIC :: WriteFieldsAMReX_PlotFile_Euler
...
CONTAINS
  ...
  SUBROUTINE WriteFieldsAMReX_PlotFile_Euler &
    ( Time, StepNo, MF_uGF_Option, MF_uCF_Option, MF_uPF_Option, &
      MF_uAF_Option, MF_uDF_Option, num_Option )
  ...
  END SUBROUTINE WriteFieldsAMReX_PlotFile_Euler
  ...
  SUBROUTINE ConvertUnits( nComp, iOS, U_plt, Field )
    ...
    ELSE IF( Field .EQ. 'AF' )THEN
      ...
      DO iComp = 1, nComp
        ...
        IF ( iComp .EQ. iAF_T .OR. iComp .EQ. iAF_Me &
            .OR. iComp .EQ. iAF_Mp .OR. iComp .EQ. iAF_Mn ) THEN 

          U_plt(:,:,:,iComp+iOS) = U_plt(:,:,:,iComp+iOS) / MeV
