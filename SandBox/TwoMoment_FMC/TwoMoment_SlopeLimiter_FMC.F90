MODULE TwoMoment_SlopeLimiterModule_FMC
    ...
    PUBLIC :: InitializeSlopeLimiter_TwoMoment_FMC
    PUBLIC :: FinalizeSlopeLimiter_TwoMoment_FMC
    PUBLIC :: ApplySlopeLimiter_TwoMoment_FMC
...
!   #endif
  ...
  CONTAINS
    ...
    SUBROUTINE InitializeSlopeLimiter_TwoMoment_FMC &
...
!   #endif
    ...
    END SUBROUTINE InitializeSlopeLimiter_TwoMoment_FMC
  
  
    SUBROUTINE FinalizeSlopeLimiter_TwoMoment_FMC
...
!   #endif
    ...
    END SUBROUTINE FinalizeSlopeLimiter_TwoMoment_FMC
  
  
    SUBROUTINE ApplySlopeLimiter_TwoMoment_FMC &
      ( iZ_B0, iZ_E0, iZ_B1, iZ_E1, GE, GX, U_M, SuppressBC_Option )
      ...
      REAL(DP), INTENT(inout)        :: &
        U_M(1:nDOFZ, &
            ...
            iZ_B1(3):iZ_E1(3),iZ_B1(4):iZ_E1(4),1:nCM,1:nSpecies)
...
!   #if defined(THORNADO_OMP_OL)
!       !$OMP TARGET ENTER DATA &
!       !$OMP MAP( to: iZ_B0, iZ_E0, GE, GX, U_M )
!   #elif defined(THORNADO_OACC)
!       !$ACC ENTER DATA ASYNC &
!       !$ACC COPYIN( iZ_B0, iZ_E0, GE, GX, U_M )
!   #endif
      ...
      IF ( TRIM( SlopeLimiterMethod ) == 'WENO' ) THEN
        ...
        CALL ApplySlopeLimiter_WENO &
               ( iZ_B0, iZ_E0, iZ_B1, iZ_E1, GE, GX, U_M )
...
!   #if defined(THORNADO_OMP_OL)
!       !$OMP TARGET EXIT DATA &
!       !$OMP MAP( release: iZ_B0, iZ_E0, GE, GX, U_M )
!   #elif defined(THORNADO_OACC)
!       !$ACC EXIT DATA ASYNC &
!       !$ACC DELETE( iZ_B0, iZ_E0, GE, GX, U_M )
...
!   #endif
    ...
    END SUBROUTINE ApplySlopeLimiter_TwoMoment_FMC
    ...
    SUBROUTINE ApplySlopeLimiter_TVD &
      ...
      INTEGER  :: &
        iZ1, iZ2, iZ3, iZ4, iCM, iS, iE_G, &
        iNodeZ, iNodeE, iNodeX
      REAL(DP) :: &
        dSlope, wSqrtGm, Alpha, uCM_K, &
        C_0, C_X1, C_X2, C_X3, &
...
!   #endif
      ...
      CALL DetectTroubledCells_TwoMoment &
             ( iZ_B0, iZ_E0, iZ_B1, iZ_E1, U_M, TroubledCell )
...
!       !$OMP          C_0, C_X1, C_X2, C_X3, uCM, uCM_K, Alpha, &
...
!       !$ACC          C_0, C_X1, C_X2, C_X3, uCM, uCM_K, Alpha, &
!       !$ACC          CL_X1, CL_X2, CL_X3, C0_L, C0_R ) &
!       !$ACC PRESENT( iZ_B0, iZ_E0, TroubledCell, WeightsX_q, GX, U_M, &
!       !$ACC          M2N_Vec_0, M2N_Vec_1, M2N_Vec_2, M2N_Vec_3 )
      ...
      DO iZ2  = iZ_B0(2), iZ_E0(2)
        ...
        IF( TroubledCell(iZ2,iZ3,iZ4,iE_G,iS) )THEN
          ...
          DO iNodeX = 1, nDOFX
            ...
            uCM(iNodeX) = U_M(iNodeZ,iZ1,iZ2,iZ3,iZ4,iCM,iS)
          END DO
          ...
          C_X3 = Zero
          DO iNodeX = 1, nDOFX
            C_0  = C_0  + N2M_Vec_0(iNodeX) * uCM(iNodeX)
            C_X1 = C_X1 + N2M_Vec_1(iNodeX) * uCM(iNodeX)
            C_X2 = C_X2 + N2M_Vec_2(iNodeX) * uCM(iNodeX)
            C_X3 = C_X3 + N2M_Vec_3(iNodeX) * uCM(iNodeX)
          END DO
          ...
          DO iNodeX = 1, nDOFX
            iNodeZ = iNodeE + ( iNodeX - 1 ) * nDOFE
            C0_L = C0_L &
                 + N2M_Vec_0(iNodeX) * U_M(iNodeZ,iZ1,iZ2-1,iZ3,iZ4,iCM,iS)
          END DO
          ...
          IF ( iZ_B0(3) /= iZ_E0(3) ) THEN
            ...
            DO iNodeX = 1, nDOFX
              iNodeZ = iNodeE + ( iNodeX - 1 ) * nDOFE
              C0_L = C0_L &
                   + N2M_Vec_0(iNodeX) * U_M(iNodeZ,iZ1,iZ2,iZ3-1,iZ4,iCM,iS)
            END DO
          ...
          IF ( iZ_B0(4) /= iZ_E0(4) ) THEN
            ...
            DO iNodeX = 1, nDOFX
              iNodeZ = iNodeE + ( iNodeX - 1 ) * nDOFE
              C0_L = C0_L &
                   + N2M_Vec_0(iNodeX) * U_M(iNodeZ,iZ1,iZ2,iZ3,iZ4-1,iCM,iS)
            END DO
          ...
          IF( dSlope > SlopeTolerance * ABS( C_0 ) )THEN
            ...
            DO iNodeX = 1, nDOFX
              ...
              uCM_K = uCM_K + wSqrtGm * uCM(iNodeX)
              ...
              Alpha = Alpha + wSqrtGm * uCM(iNodeX)
            ...
            IF( ABS( Alpha ) > Zero )THEN
              ...
              DO iNodeX = 1, nDOFX
                ...
                U_M(iNodeZ,iZ1,iZ2,iZ3,iZ4,iCM,iS) = uCM(iNodeX) * uCM_K / Alpha
...
!   #endif
  ...
  END MODULE TwoMoment_SlopeLimiterModule_FMC