...
We assume that the source term $\vect{S}(\vect{U}_{k})$ is non-stiff so that the equations can integrated efficiently with explicit methods.  
...
To summarize the RK-DG algorithm we let $\bar{\vect{U}}$ denote the global solution vector containing all the unknowns in $D$ and $\bar{\vect{F}}$ denote the corresponding DG discretization from the right-hand side of Eq.~\eqref{eq:semidiscreteDiscretized}, so that the global ODE system can be written as the $d_{t}\bar{\vect{U}}=\bar{\vect{F}}(\bar{\vect{U}})$.  
We denote the TVD limiter with $\Lambda^{\TVD}\{\}$ and the `positivty' limiter preventing unphysical states with $ \Lambda^{\Pos}\{\}$.  
(E.g., application of the TVD limiter is $\bar{\vect{U}} := \Lambda^{\TVD}\{\bar{\vect{U}}\}$.)  
...
$\Delta t=\min_{D}(\Delta t_{\TVD},\Delta t_{\Pos})$
\begin{equation}
  \Delta t_{\TVD} = \f{1}{d}\,\f{1}{(2k+1)}\min_{i}\big(\Delta x^{i}/|\lambda^{i}|\big)\quad\text{and}\quad
  \Delta t_{\Pos} = \f{\hat{w}_{N}}{d}\min_{i}\big(\Delta x^{i}/|\lambda^{i}|\big)
\end{equation}
...
To limit the scope of this initial study, we use polynomials of degree $k=2$ (except in Section~\ref{sec:isentropicVortex}, where we also use $k=1$ and $k=3$) combined with SSP-RK3 time stepping, and run all the tests with a Courant-Friedrichs-Lewy (CFL) factor of $C=0.1$.  
...
\begin{figure}[h]
  ...
  \end{minipage}
  \caption{\label{fig:SodSedov}{\it Upper left panel:} numerical solution of Sod's problem using $100$ elements (plusses) compared with the exact solution (solid lines; from \cite{toro_1999}).  {\it Upper right panel:} numerical solution of the Sedov-Taylor blast wave using $256$ elements (plusses) against the exact solution (solid lines).  In the lower panels we plot locations in the $xt$-plane of elements flagged for limiting ($I_{\bK}>C_{\TCI}$) by the Sod (left) and Sedov (right) problems.}
\end{figure}
...
\begin{wrapfigure}{l}{2.75in}
  \centering
  \includegraphics[width=16pc]{./Figures/KelvinHelmholtz_TCI_24_Astronum_2018}
