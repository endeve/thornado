...
\begin{abstract}
We develop implicit-explicit (IMEX) schemes for neutrino transport through a background material in the \texttt{thornado} code in the context of a two-moment mode that evolves the angular moments of a neutrino phase-space distribution function.
...
Several algebraic closures are compared with these bounds in mind and the Cernohorsky and Bludman closure\cite{cernohorskyBludman_1994}, which preserves the bounds, is chosen for our IMEX schemes.
...
PD-ARS denotes a convex-invariant IMEX Runge-Kutta scheme that has first-order accuracy in general, high-order accuracy in the streaming limit, and works well in the diffusion limit.
...
The accuracy and convex-invariance of our PD-ARS schemes are demonstrated in the numerical tests with a first-order discontinuous Galerkin(DG) method for spatial discretization and a simple Lax-Friedrichs flux.
