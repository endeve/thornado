...
\end{align}
where $\eta := \dfrac{\dt}{\dx}$. The first two terms can form a 3-term convex combination by requiring $\eta \in [0,1]$.
It can be shown that $\bcM_{i}^{n+1}$ remains realizable given $\bcM_{i}^{n}$ is realizable, $\eta \in [0,1]$ and $\cC(\bcM)$ is positivity-preserving. The proof can be found in \cite{
chu_2018}.
