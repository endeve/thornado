...
In the streaming limit, the PD-ARS schemes reduce to SSPRK schemes, such that PD-ARS2 to SSPRK2 and PD-ARS3 to SSPRK3, respectively.
...
Since these are special cases for $N=8$ and $N=16$, we won't recommend PD-ARS2 instead of SSP2332 for the damping problem.
...
SSP2332 and IMEX PC2 failed after a few time step with neither CB or Minerbo closure because of unrealizable moment.
Even though IMEXRKCB2 with Minerbo closure can run and reach a steady state in this test, its results are different from that of PD-ARS schemes with CB closure and there is no guarantee for its stability.
...
In the middle and right column, we can see how the neutrinos are generated near the core, stream out, and reach an equilibrium distribution over the phase space eventually.
The oscillation in the right column is caused by numerical error.
%can see how the scattering reshapes the flux direction. 
%The oscillatory behavior, which is more obvious for high-energy groups, is due to numerical error.
%by the oscillatory in $|\bcH|$ as a result of scattering: scattering changes neutrino's direction and brings randomness to $|\bcH|$.  
%High-energy groups can gain more randomness due to machine accuracy error.
