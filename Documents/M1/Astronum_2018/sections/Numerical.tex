...
To compare the numerical results to analytic solutions, the averaged absolute error or the averaged relative error are computed in the $L^{1}$-error norm. We compute the absolute error for the streaming test and the diffusion test and the relative error for the damping test.
...
We see the errors obtained with (SSPRK3) and (PD-ARS with SSPRK3) are smallest and decrease as $N^{-3}$, as expected for a scheme combining third-order accurate time stepping with third-order accurate spatial discretization.
...
In the streaming limit, the PD-ARS schemes reduce to SSPRK schemes, such that (PD-ARS with SSPRK2) to SSPRK2 and (PD-ARS with SSPRK3) to SSPRK3, respectively.
...
\begin{figure}[h]
  \centering
   ...
   \caption{Absolute error versus number of elements, $N$, for the streaming sine wave test.  Results employing various time stepping schemes are compared: SSPRK2 (cyan triangles pointing up), SSPRK3 (cyan triangles pointing down), SSP2332 (green crosses), PD-ARS with SSPRK2 (light red circles) and PD-ARS with SSPRK3 (light red asterisks). Black dashed reference lines are proportional to $N^{-2}$ (top), and $N^{-3}$ (bottom), respectively.}
...
A periodic domain $D=\{x:x\in[0,1]\}$ and initial condition $\cJ_{0} = \bcH_{0} = 0.5+0.49\times\sin\big(2\pi\,x\big)$ is used.
...
For $\sigma_{\Ab}=0.1$, PD-ARS with SSPRK2 is the most accurate schemes for $N=8$ and $N=16$.
Only SSP2332, a second-order accurate scheme, displays a second-order convergence rate.  
The PD-ARS schemes are first-order accurate in the damping test as expected. 

\begin{figure}[h]
  \centering
    \includegraphics[width=0.9\textwidth]{figures/SineWaveDamping}
   \caption{Relative error versus number of elements, $N$, for the damping sine wave test. Results for different values of the absorption opacity $\sigma_{\Ab}$, employing various IMEX time stepping schemes, are compared.  Errors for $\sigma_{\Ab}=0.1$, $1$, and $10$ are plotted with red, green, and blue lines, respectively.  The IMEX schemes employed are SSP2332 ($+$), PD-ARS with SSPRK2 (triangles pointing up) and PD-ARS with SSPRK3 (triangles pointing down).  Black dashed reference lines are proportional to $N^{-1}$ (top) and $N^{-2}$ (bottom), respectively.}
  \label{fig:SineWaveDamping}
\end{figure}

\subsubsection{Sine Wave Diffusion}

The final test with known smooth solution, adopted from~\cite{radice_etal_2013}, is the sine wave diffusion test, i.e. $\sigma_{\Ab} = 0$ and $f_0 = 0$.
...
For $\sigma=10^{2}$, the errors on the number density $\cJ$ do not drop below $10^{-6}$ because of differences between the two-moment model and the diffusion equation used to obtain the analytic solution.
...
PD-ARS with SSPRK2 behaves as well as SSP2332 in the diffusion region but requires 33\% less implicit solves per time step.  
...
In this section we consider a more ``realistic'' test: two-dimensional Newtonian multigroup neutrino transport with emission, absorption, and isoenergetic scattering through a stationary background.  
...
The other schemes crashed after a few time steps, when moments became non-realizable and the closure procedure failed.  
...
In the middle column, we can see how the neutrinos are generated near the core, stream out, and reach a equilibrium distribution over the phase space eventually.
In the right column, we can see how the scattering reshapes the flux direction. 
The oscillatory behavior which is more obvious on high energy group is caused by the machine accuracy error.
