...
In the left panel in Figure~\ref{fig:NeutrinoStationaryTestEOS} we plot the thermal state of the background, which mimic a collapsed stellar core: 
\begin{subequations}
  \begin{align}
...
\end{subequations}
where the radius $r=\sqrt{(x^{1})^{2}+(x^{2})^{2}}$ is in kilometers.  
In the right panel in Figure~\ref{fig:NeutrinoStationaryTestEOS} we plot the neutrino opacities, computed by interpolation of a pre-calculated opacity table based on~\cite{Bruenn_1985}.
