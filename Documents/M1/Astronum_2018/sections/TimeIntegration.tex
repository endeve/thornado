...
Matrices $\tilde{A}$ and $A$ are both lower triangular matrices.
...
By requiring the IMEX scheme be GSA, $\vect{u}^{n+1} = \vect{u}^{(s)}$.
...
Hence $\vect{u}^{(i)}$ and $\vect{u}^{n+1}$ are realizable and the IMEX is constraint-preserving.
...
Unfortunately, no coefficients can be found for standard IMEX schemes under the constraint-preserving constraints and the accuracy constraints for 2nd-order or higher. 
...
To keep things simple, we aim for 1st-order in general, high-order in streaming limit, diffusion accurate, constraint-preserving IMEX schemes.

\subsection{Diffusion Accurate, Constraint-Preserving Implicit-Explicit Schemes}

Diffusion accurate is an important property the time integration should have.
...
\rc{It can be rewrite to $c=\tilde{c}$.}
We have proved in \cite{chu_etal_2018} that only type~ARS IMEX schemes can be diffusion accurate and constraint-preserving at the same time.

\subsection{PD-ARS schemes}

Combining the conditions we have, the 1st-order in general, high-order in streaming limit, diffusion accurate, constraint-preserving IMEX schemes we need are type~ARS IMEX schemes satisfying the following constraints:
\begin{enumerate}
    ...
    \end{equation}
    \item Diffusion accurate: \rc{$c=\tilde{c}$.}
    \begin{equation}
      \vect{e}_{i}^{T}A^{-1}\tilde{A}\,\vect{e} = 1, \quad i=2,\ldots,s.
