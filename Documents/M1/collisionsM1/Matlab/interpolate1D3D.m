function [ Interpolant ] = interpolate1D3D( x1, x2, x3, x4, x11D, x21D, x31D, x41D, Logflag, V4D,OS )
% function [ Interpolant ] = 
%          interpolate1D3D( x1, x2, x3, x4, x11D, x21D, x31D, x41D, Logflag, V4D,OS )
% x1 is a (N,1) array 
% x2, x3, x4 are (M,1) array 
% function value Interpolant is a (N,M) array

Interpolant = zeros( size( x1, 1 ), size( x2, 1) );

for j = 1 : size( x2, 1 )
    
    ix2 = find( x21D < x2(j), 1, 'last' );
    ix3 = find( x31D < x3(j), 1, 'last' );
    ix4 = find( x41D < x4(j), 1, 'last' );
    
    if ( Logflag(2) == 1 ) dx2 = log10( x2(j) / x21D(ix2) ) / log10( x21D(ix2+1) / x21D(ix2) );
    elseif ( Logflag(2) == 0 ) dx2 = ( x2(j) - x21D(ix2) ) / ( x21D(ix2+1) - x21D(ix2) );
    end
    
    if ( Logflag(3) == 1 ) dx3 = log10( x3(j) / x31D(ix3) ) / log10( x31D(ix3+1) / x31D(ix3) );
    elseif ( Logflag(3) == 0 ) dx3 = ( x3(j) - x31D(ix3) ) / ( x31D(ix3+1) - x31D(ix3) );
    end
    
    if ( Logflag(4) == 1 ) dx4 = log10( x4(j) / x41D(ix4) ) / log10( x41D(ix4+1) / x41D(ix4) );
    elseif ( Logflag(4) == 0 ) dx4 = ( x4(j) - x41D(ix4) ) / ( x41D(ix4+1) - x41D(ix4) );
    end
