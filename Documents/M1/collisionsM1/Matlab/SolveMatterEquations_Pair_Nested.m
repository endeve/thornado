function [J, J0, D, T, Y, E, iter, iter_in] = SolveMatterEquations_Pair_Nested( Jin, dt, Chi, D, T, Y, E)
...
while((~CONVERGED)&&(k<=maxIter))
    ...
    while((~CONVERGED_IN)&&(k_in<=maxIter))
        ...
        Jnew.Ne = (Jin.Ne + Chi.Ne.*J0.Ne + eta_NES.Ne + eta_TP.Ne)./(1 + Chi.Ne + Chi_NES.Ne + Chi_TP.Ne);
        Jnew.ANe = (Jin.ANe + Chi.ANe.*J0.ANe + eta_NES.ANe + eta_TP.ANe)./(1 + Chi.ANe + Chi_NES.ANe + Chi_TP.ANe);
