...
\title{A Nodal Discontinuous Galerkin Method for the Multi-Group Two-Moment Model of Neutrino Transport}
...
\begin{abstract}
We present a nodal Discontinuous Galerkin (DG) method for solving the non-relativistic multi-group two-moment model for neutrino transport.  
The method --- ultimately targeted for simulations of core-collapse supernovae --- includes the use of curvilinear coordinates and non-ideal microphysics (nuclear equation of state and weak interactions) for neutrino-matter interactions.  We give implementation details and present results from an extensive set of benchmark problems in one spatial dimension. 
\end{abstract}
...
\begin{figure}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Results from solving the multi-group Homogeneous sphere problem with the DG(2)+SIRK2 method using $100$ spatial elements and $24$ energy groups, and using material properties from Model~B in Table~\ref{tab:tabulatedModels} to compute the neutrino absorption opacities.
...
For the temperature profile we set $T_{\mbox{\tiny min}}=5\times10^{9}$~K, $T_{\mbox{\tiny max}}=2.6\times10^{11}$~K, $R_{T}=25$~km, and $H_{T}=20$~km.  
For the electron fraction profile we set $Y_{e,\mbox{\tiny min}}=0.4$, $Y_{e,\mbox{\tiny max}}=0.46$, $R_{Y_{e}}=45$~km, and $H_{Y_{e}}=10$~km.  
...
In Figure~\ref{fig:deleptonizationProblemFluid} we plot fluid quantities versus radius at select times: $t=0$~ms (initial condition, dotted lines), $t=3$~ms (dash-dot), $t=10$~ms (dashed), and $t=100$~ms (solid).  
The results were obtained with the DG(1)+SIRK2 scheme using 200 radial elements and 30 energy bins.  
We plot the electron fraction (upper left panel); mass fractions of protons and neutrons (black), alpha particles (red), and heavy nuclei (blue) (upper right panel); temperature (lower left); and entropy per baryon (lower right).  
...
\begin{figure}
  \begin{center}
	...
	\end{tabular}
  \end{center}
  \caption{Fluid quantities from the deleptonization problem.  Plotted versus radius for various times are: electron fraction (top left), mass fractions of protons plus neutrons (black), alpha particles (red), and the mean heavy nucleus (blue; top right), temperature (bottom left), and entropy per baryon (bottom right).}
  \label{fig:deleptonizationProblemFluid}
\end{figure}

In Figure~\ref{fig:deleptonizationProblemRadiation} we plot radiation quantities versus radius at the same times a in Figure~\ref{fig:deleptonizationProblemFluid}.  
...
\begin{figure}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Radiation quantities from the deleptonization problem.  Plotted versus radius for various times are neutrino number density (top left), flux factor (top right), mean energy (bottom left), and luminosity (bottom right).  The radii where the optical depth $\tau=2/3$ are indicated with vertical lines.}
...
\begin{figure}
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  \caption{Results for the deleptonization problem where the spatial and spectral resolution has been varied: the electron fraction (left panel) and mean energy (right panel) are plotted versus radius for $t=100$~ms.  The black solid line is obtained with the DG(1)+SIRK2 scheme using $N_{r}\times N_{\epsilonNu}=200\times30$ elements, while blue circles represent results obtained with the DG(2)+SIRK2 scheme using $64\times12$ elements.  }
  \label{fig:}
\end{figure}

\section{Summary}

We have presented details of the DG scheme the two-moment model for neutrino transport in \thornado.  

Test problems.  

A version of the code has been made publicly available on GitHub\footnote{\url{https://github.com/ECP-Astro/thornado_mini}}.  
This release includes the deleptonization problem presented in Section~\ref{sec:deleptonizationProblem}.  
