...
&  \eta(\epsilonNu,\bU) - \tilde{\chi}(\epsilonNu,\bU)f(\omegaNu,\epsilonNu,\bx,t) \nonumber \\
& + \dfrac{1}{c(hc)^3} \epsilonNu^2 \int d\mu'_0 R_{\IS}( \omega', \omega,\epsilonNu, \bU) f(\omega', \epsilonNu,\bx,t) \nonumber \\
& - \dfrac{1}{c(hc)^3} \epsilonNu^2 f(\omegaNu,\epsilonNu,\bx,t)\int d\omega' R_{\IS}(\omega, \omega', \epsilonNu,\bU) \nonumber \\
& + \dfrac{1}{c(hc)^3} [1- f(\omegaNu,\epsilonNu,\bx,t)] \int d\epsilonNu^{\prime} \epsilonNu^{\prime 2} \int d\omega' R^{\In}_{\NES} (\omega,\omega',\epsilonNu,\epsilonNu^{\prime},\bU)f(\omega',\epsilonNu^{\prime},\bx,t ) \nonumber \\
& - \dfrac{1}{c(hc)^3} f(\omegaNu,\epsilonNu,\bx,t) \int d\epsilonNu^{\prime} \epsilonNu^{\prime 2} \int d\mu' R^{\Out}_{\NES} (\omega,\omega',\epsilonNu,\epsilonNu^{\prime},\bU) [1-f(\omega',\epsilonNu^{\prime},\bx,t )].
\end{align}
...
The isotropic elastic scattering kernel $R_{\IS}(\omega, \omega', \epsilonNu,\bU)$ has the following relation:
\begin{align}
R_{\IS}(\omega, \omega', \epsilonNu,\bU) & = R_{\IS}^{nucleons}(\omega, \omega', \epsilonNu,\bU) +  R_{\IS}^{nuclei}(\omega, \omega', \epsilonNu,\bU),
\end{align}
where
\begin{align}
\cos\theta = \omega\omega' + [(1-\omega^2)(1-\omega^{\prime 2})]^{1/2}\cos\theta,
\end{align}
and
\begin{align}
R_{\IS}(\cos\theta, \epsilonNu, \bU) = R^{nucleons}(\cos\theta,\epsilonNu, \bU) + R^{nuclei}_{\IS}(\cos\theta, \epsilonNu, \bU)
\end{align}
where $R^{nucleons}_{\IS} (\cos\theta,\epsilonNu, \bU)$ and $R^{nuclei}_{\IS}(\cos\theta,\epsilonNu,\bU)$ are given by Eq. (35) and Eq. (36) in \citet{mezzacappaBruenn_1993a}, respectively.

Expanding the isotropic elastic scattering kernel in the first two terms of a Legendre series yields
\begin{align}
R_{\IS}(\cos\theta,\epsilonNu) \approx \dfrac{1}{2} \Phi_{0,\IS}(\epsilonNu) + \dfrac{3}{2}\Phi_{1,\IS}(\epsilonNu)\cos\theta
\end{align}
...
Expand the neutrino-electron out scattering kernel $R^{\Out}_{\NES}(\epsilonNu,\epsilonNu', \omega)$ in a Legendre series:
\begin{align}
R^{\Out}_{\NES}(\epsilonNu,\epsilonNu', \omega, \bU) = \sum_{l=0}^{\infty} \dfrac{2l+1}{2} R^{\Out}_{{\NES}, l}(\epsilonNu,\epsilonNu', \bU) P_l(\omega),
\end{align}
where the Legendre coefficients $R^{\Out}_{{\NES}, l}(\epsilonNu,\epsilonNu')$ are given by
\begin{align}
R^{\Out}_{{\NES}, l}(\epsilonNu,\epsilonNu', \bU) = \int_{-1}^{1} d\omega P_l(\omega) R^{\Out}_{\NES}(\epsilonNu,\epsilonNu', \omega, \bU).
\end{align}
   
Expanding the neutrino-electron scattering kernels in the first two terms of Legendre series yields
\begin{align}
R^{\In/\Out}_{\NES}(\epsilonNu, \epsilonNu^{\prime}, \cos\theta, \bU) & = \sum_{l=0}^{1} \dfrac{ 2l+1}{2}\Phi^{\In/\Out}_{l,\NES}(\epsilonNu,\epsilonNu^{\prime},\bU)P_l(\cos\theta) \\
& \approx \dfrac{1}{2}\Phi_{0,\NES}^{\In/\Out}(\epsilonNu,\epsilonNu^{\prime}, \bU) + \dfrac{3}{2}\Phi^{\In/\Out}_{1,\NES}(\epsilonNu,\epsilonNu^{\prime}, \bU)\cos\theta
\end{align}

The isotropic term $\dfrac{1}{2}\Phi_{0,\NES}^{\In/\Out}(\epsilonNu,\epsilonNu^{\prime}, \bU)$ is the $R^{\In}(\epsilonNu,\epsilonNu')$ and $R^{\Out}(\epsilonNu,\epsilonNu')$  in Eqs.~\eqref{eq:momentEquationEnergy} and \eqref{eq:momentEquationMomentum}. 
