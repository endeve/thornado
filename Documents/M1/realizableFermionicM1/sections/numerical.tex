...
We are not concerned with moment realizability here, and we do not apply the realizability-enforcing limiter in these tests.  
...
Here, this test is computed on a three-dimensional Cartesian domain $D=\{\vect{x}\in\bbR^{3}:x^{1}\in[0,2], x^{2}\in[0,2], x^{3}\in[0,2]\}$.  
...
In all the tests, we use the IMEX scheme PD-ARS --- the least compute-intensive of the constraint-preserving IMEX schemes presented here.  
...
The particle density $\cJ$ and the flux factor $h=|\vect{\cH}|/\cJ$ (left and right panels, respectively) are plotted versus radius $r=|\vect{x}|$ using scatter plots.  
