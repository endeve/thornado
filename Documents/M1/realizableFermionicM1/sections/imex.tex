...
Such that, $c_{i0} = 1 - \sum_{j=1}^{i-1}c_{ij}$, $c_{ij} = \sum_{l=j}^{i-1}a_{il}b_{lj}$, $\tilde{c}_{i0} = \tilde{a}_{i1} + \sum_{j=2}^{i-1} a_{ij}\tilde{b}_{j1}$,$\tilde{c}_{ij} = \tilde{a}_{ij} + \sum_{l=j+1}^{i-1} a_{il}\tilde{b}_{lj}$ $b_{ij} = - \frac{1}{a_{ii}} \sum_{l = j}^{i-1}a_{il}b_{lj}$, $b_{ii} = \frac{1}{a_{ii}}$, $\tilde{b}_{ij} = \frac{1}{a_{ii}}\big( -\tilde{a}_{ij} - \sum_{l = j+1}^{i-1}a_{il}\tilde{b}_{lj}\big)$. 
...
Importantly, the coefficients $c_{ij}$ in Eq.~\eqref{eq:imexStagesRewrite} satisfy $\sum_{j=0}^{i-1}c_{ij}=1$, which implies the so-called Shu-Osher form.  
...
(It can be shown that IMEX schemes of type~A satisfying the conditions we list below Eq.~\eqref{eq:orderConditionsEx} do not exist.)
