...
\end{define}
As an example, given a spatial discretization of a model that is constraint-preserving with the forward Euler method for a sufficiently small time step, high-order strong stability-preserving Runge-Kutta (SSP-RK) methods, which can be expressed as convex combinations of forward Euler steps, form a class of constraint-preserving schemes.  
...
Unfortunately, the ``diffusion limit requirement" in Eq.~\eqref{eq:diffusionCondition}, together with the order conditions given by Eqs.~\eqref{orderConditions1} and \eqref{orderConditions2} (with $\alpha=0$), and the positivity conditions on $c_{ij}$ and $\tilde{c}_{ij}$, result in too many constraints to obtain a constraint-preserving IMEX scheme.
...
Scheme PC2 is second-order in the streaming limit, has been demonstrated to work well in the diffusion limit \cite{mcclarren_etal_2008,radice_etal_2013}, and satisfies the positivity conditions.  
