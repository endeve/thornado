...
In \ref{app:butcherTables}, we provide another example of a type A scheme (scheme PA2+), with a significantly larger $c_{\mbox{\tiny Sch}}$.  
...
To reduce the number of constraints, we relax the requirement of overall second order accuracy of the IMEX scheme.  
Instead, we only require the scheme to be second order accurate in the streaming limit ($\vect{\cQ}=0$).  
This is similar to the IMEX scheme given in \cite{mcclareen_2008} (see scheme PC2 in \ref{app:butcherTables}).  