...
5. `buildRun.all.sh` inside `$HOME/ExaStar/thornado/SandBox/TwoMoment_OrderV/Executables` is the script to build and/or run both sineWaveStreaming and relaxation case with a grid a {8,8,8} and {16,16,16}
7. Latest weaklib is needed and please overwrite the original $WEAKLIB_HOME/Distributions/Library/wlInterpolationModule.F90 with wlInterpolationModule.F90 in the root directory of ms69-merging branch. 
