MODULE InputOutputModuleHDF
  ...
  USE ProgramHeaderModule, ONLY: &
    ProgramName, &
    nE, swE, nNodesE, nDOFE, &
    nX, swX, nNodesX, nDOFX, &
    nDOF, &
    iX_B0, &
    iX_B1, &
    iX_E0, &
...
CONTAINS
  ...
  SUBROUTINE WriteGeometryFieldsHDF( Time, WriteGhost )
    ...
    IF( WriteGhost )THEN
      ...
      CALL WriteDataset1DHDF &
             ( MeshX(1) % Center(iX_B1(1):iX_E1(1)) / U % LengthX1Unit, &
               DatasetName, FILE_ID )

      DatasetName = TRIM( GroupName ) // '/X2_C'

      CALL WriteDataset1DHDF &
             ( MeshX(2) % Center(iX_B1(2):iX_E1(2)) / U % LengthX2Unit, &
               DatasetName, FILE_ID )

      DatasetName = TRIM( GroupName ) // '/X3_C'

      CALL WriteDataset1DHDF &
             ( MeshX(3) % Center(iX_B1(3):iX_E1(3)) / U % LengthX3Unit, &
               DatasetName, FILE_ID )
      ...
      CALL WriteDataset1DHDF &
             ( MeshX(1) % Width(iX_B1(1):iX_E1(1)) / U % LengthX1Unit, &
               DatasetName, FILE_ID )

      DatasetName = TRIM( GroupName ) // '/dX2'

      CALL WriteDataset1DHDF &
             ( MeshX(2) % Width(iX_B1(2):iX_E1(2)) / U % LengthX2Unit, &
               DatasetName, FILE_ID )

      DatasetName = TRIM( GroupName ) // '/dX3'

      CALL WriteDataset1DHDF &
             ( MeshX(3) % Width(iX_B1(3):iX_E1(3)) / U % LengthX3Unit, &
               DatasetName, FILE_ID )
    ...
    ELSE
      ...
      CALL WriteDataset1DHDF &
             ( NodeCoordinates(MeshX(1),iX_B0(1),iX_E0(1),nNodesX(1)) &
                 / U % LengthX1Unit, DatasetName, FILE_ID )

      DatasetName = TRIM( GroupName ) // '/X2'

      CALL WriteDataset1DHDF &
             ( NodeCoordinates(MeshX(2),iX_B0(2),iX_E0(2),nNodesX(2)) &
                 / U % LengthX2Unit, DatasetName, FILE_ID )
      ...
      CALL WriteDataset1DHDF &
             ( MeshX(1) % Center(iX_B0(1):iX_E0(1)) / U % LengthX1Unit, &
               DatasetName, FILE_ID )

      DatasetName = TRIM( GroupName ) // '/X2_C'

      CALL WriteDataset1DHDF &
             ( MeshX(2) % Center(iX_B0(2):iX_E0(2)) / U % LengthX2Unit, &
               DatasetName, FILE_ID )

      DatasetName = TRIM( GroupName ) // '/X3_C'

      CALL WriteDataset1DHDF &
             ( MeshX(3) % Center(iX_B0(3):iX_E0(3)) / U % LengthX3Unit, &
               DatasetName, FILE_ID )
      ...
      CALL WriteDataset1DHDF &
             ( MeshX(1) % Width(iX_B0(1):iX_E0(1)) / U % LengthX1Unit, &
               DatasetName, FILE_ID )

      DatasetName = TRIM( GroupName ) // '/dX2'

      CALL WriteDataset1DHDF &
             ( MeshX(2) % Width(iX_B0(2):iX_E0(2)) / U % LengthX2Unit, &
               DatasetName, FILE_ID )

      DatasetName = TRIM( GroupName ) // '/dX3'

      CALL WriteDataset1DHDF &
             ( MeshX(3) % Width(iX_B0(3):iX_E0(3)) / U % LengthX3Unit, &
               DatasetName, FILE_ID )
        ...
        CALL WriteDataset3DHDF &
               ( Field3D &
                   ( uGF(1:nDOFX, &
                         iX_B1(1):iX_E1(1), &
                         iX_B1(2):iX_E1(2), &
                         iX_B1(3):iX_E1(3),iGF), nX+2*swX, nNodesX, &
        ...
        CALL WriteDataset3DHDF &
               ( Field3D &
                   ( uGF(1:nDOFX, &
                         iX_B0(1):iX_E0(1), &
                         iX_B0(2):iX_E0(2), &
                         iX_B0(3):iX_E0(3),iGF), nX, nNodesX, &
                     nDOFX, NodeNumberTableX ) / unitsGF(iGF), &
                 DatasetName, FILE_ID )
        ...
        CALL WriteDataset3DHDF &
               ( Field3D &
                   ( uCF(1:nDOFX, &
                         iX_B1(1):iX_E1(1), &
                         iX_B1(2):iX_E1(2), &
                         iX_B1(3):iX_E1(3),iFF), nX+2*swX, nNodesX, &
        ...
        CALL WriteDataset3DHDF &
               ( Field3D &
                   ( uCF(1:nDOFX, &
                         iX_B0(1):iX_E0(1), &
                         iX_B0(2):iX_E0(2), &
                         iX_B0(3):iX_E0(3),iFF), nX, nNodesX, &
                     nDOFX, NodeNumberTableX ) / unitsCF(iFF), &
                 DatasetName, FILE_ID )
  ...
  SUBROUTINE WriteRadiationFieldsHDF( Time, WriteGhost )
    ...
    IF( WriteGhost )THEN
      ...
      CALL WriteDataset1DHDF &
             ( MeshE % Center(iE_B1:iE_E1) / U % EnergyUnit, &
               DatasetName, FILE_ID )
      ...
      CALL WriteDataset1DHDF &
             ( MeshE % Width(iE_B1:iE_E1) / U % EnergyUnit, &
               DatasetName, FILE_ID )
    ...
    ELSE
      ...
      CALL WriteDataset1DHDF &
             ( MeshE % Center(iE_B0:iE_E0) / U % EnergyUnit, &
               DatasetName, FILE_ID )
      ...
      CALL WriteDataset1DHDF &
             ( MeshE % Width(iE_B0:iE_E0) / U % EnergyUnit, &
               DatasetName, FILE_ID )
          ...
          CALL WriteDataset3DHDF &
               ( Field3D &
                   ( uGR(1:nDOFX, &
                         ...
                         iX_B0(2):iX_E0(2), &
                         iX_B0(3):iX_E0(3),iRF,iS), &
                     nX + 2*swX, nNodesX, nDOFX, NodeNumberTableX ) / unitsGR(iRF), &
                 DatasetName, FILE_ID )
    ...
    DO iRF = 1, nDR
      ...
      IF( WriteGhost )THEN
        ...
        CALL WriteDataset3DHDF &
               ( uDR(iX_B1(1):iX_E1(1), &
                     iX_B1(2):iX_E1(2), &
                     iX_B1(3):iX_E1(3),iRF), DatasetName, FILE_ID )
      ...
      ELSE
        ...
        CALL WriteDataset3DHDF &
               ( uDR(iX_B0(1):iX_E0(1), &
                     iX_B0(2):iX_E0(2), &
                     iX_B0(3):iX_E0(3),iRF), DatasetName, FILE_ID )
