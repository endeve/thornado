...
CONTAINS
  ...
  SUBROUTINE ReadCheckpointFile( iChkFile )
    ...
    CALL ReadMultiFabData( nLevels-1, pCM, 1, iChkFile )
  ...
  SUBROUTINE WriteFieldsAMReX_PlotFile &
    ( Time, StepNo, MF_uGF_Option, MF_uCM_Option, MF_uPM_Option, &
    ...
    TYPE(amrex_multifab), INTENT(in), OPTIONAL :: MF_uCM_Option(0:nLevels-1)
    TYPE(amrex_multifab), INTENT(in), OPTIONAL :: MF_uPM_Option(0:nLevels-1)
    TYPE(amrex_multifab), INTENT(in), OPTIONAL :: MF_uAM_Option(0:nLevels-1)
    TYPE(amrex_multifab), INTENT(in), OPTIONAL :: MF_uDM_Option(0:nLevels-1)

    CHARACTER(08)                   :: NumberString
    CHARACTER(64)                   :: PlotFileName
    LOGICAL                         :: WriteGF
    LOGICAL                         :: WriteMFF_C, WriteMFF_P, &
                                       WriteMFF_A, WriteMFF_D
    ...
    IF( WriteMFF_C )THEN
      ...
      DO iComp = 1, nCM
        ...
        CALL amrex_string_build &
               ( VarNames( iComp + iOS ), TRIM( ShortNamesCM(iComp) ) )
    ...
    IF( WriteMFF_P )THEN
      ...
      DO iComp = 1, nPM
        CALL amrex_string_build &
               ( VarNames( iComp + iOS ), TRIM( ShortNamesPM(iComp) ) )
      END DO
    ...
    IF( WriteMFF_A )THEN
      ...
      DO iComp = 1, nAM
        ...
        CALL amrex_string_build &
               ( VarNames( iComp + iOS ), TRIM( ShortNamesAM(iComp) ) )
    ...
    IF( WriteMFF_D )THEN
      ...
      DO iComp = 1, nDM
        ...
        CALL amrex_string_build &
               ( VarNames( iComp + iOS ), TRIM( ShortNamesDM(iComp) ) )
    ...
    DO iLevel = 0, nLevels-1
      ...
      IF( WriteMFF_C )THEN
        ...
        CALL ComputeCellAverage_MF &
          ( nCM, MF_uCM_Option(iLevel), MF_uGF_Option(iLevel), &
            iOS, iOS_CPP, 'CM', MF_plt(iLevel) )
      ...
      IF( WriteMFF_P )THEN
        ...
        CALL ComputeCellAverage_MF &
          ( nPM, MF_uPM_Option(iLevel), MF_uGF_Option(iLevel), &
            iOS, iOS_CPP, 'PM', MF_plt(iLevel) )
      ...
      IF( WriteMFF_A )THEN
        ...
        CALL ComputeCellAverage_MF &
          ( nAM, MF_uAM_Option(iLevel), MF_uGF_Option(iLevel), &
            iOS, iOS_CPP, 'AM', MF_plt(iLevel) )
      ...
      IF( WriteMFF_D )THEN
        ...
        CALL ComputeCellAverage_MF &
          ( nDM, MF_uDM_Option(iLevel), MF_uGF_Option(iLevel), &
            iOS, iOS_CPP, 'DM', MF_plt(iLevel) )
  ...
  SUBROUTINE ComputeCellAverage_MF &
    ...
    DO WHILE( MFI % next() )
      ...
      !PRINT*, 'unitsGF: ', unitsGF
      !PRINT*, 'ShortNamesCM: ', ShortNamesCM
  ...
  SUBROUTINE ConvertUnits( nComp, iOS, U_plt, Field )
    ...
    ELSE IF( Field .EQ. 'CM' )THEN
      ...
      DO iComp = 1, nComp
        ...
        !PRINT*, 'iComp: ', iComp
        !PRINT*, 'unitsCM: ', unitsCM
        !PRINT*, 'unitsCM(iComp): ', unitsCM(iComp)

        U_plt(:,:,:,iComp+iOS) = U_plt(:,:,:,iComp+iOS) / unitsCM(iComp)

      END DO

    ELSE IF( Field .EQ. 'PM' )THEN

      DO iComp = 1, nComp

        U_plt(:,:,:,iComp+iOS) = U_plt(:,:,:,iComp+iOS) / unitsPM(iComp)

      END DO

    ELSE IF( Field .EQ. 'AM' )THEN

      DO iComp = 1, nComp

        U_plt(:,:,:,iComp+iOS) = U_plt(:,:,:,iComp+iOS) / unitsAM(iComp)

      END DO

    ELSE IF( Field .EQ. 'DM' )THEN

      DO iComp = 1, nComp

        U_plt(:,:,:,iComp+iOS) = U_plt(:,:,:,iComp+iOS) / unitsDM(iComp)
