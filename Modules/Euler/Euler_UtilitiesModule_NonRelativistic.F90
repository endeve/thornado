...
CONTAINS
  ...
  SUBROUTINE ComputePrimitive_Vector &
    ...
    DO iNX = 1, SIZE( N )
      ...
      D  (iNX) = N  (iNX)
      V_1(iNX) = S_1(iNX) / ( Gm_dd_11(iNX) * N(iNX) )
      V_2(iNX) = S_2(iNX) / ( Gm_dd_22(iNX) * N(iNX) )
      V_3(iNX) = S_3(iNX) / ( Gm_dd_33(iNX) * N(iNX) )
      E  (iNX) = G  (iNX) - Half * ( S_1(iNX)**2 / Gm_dd_11(iNX) &
